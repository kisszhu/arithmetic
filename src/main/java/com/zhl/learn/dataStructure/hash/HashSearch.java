package com.zhl.learn.dataStructure.hash;

/**
 * Created by zhuhailong-dc on 2018/3/12.
 * 实现一个哈希表，要求
 * a.哈希函数采用除留余数法，即f(key)=key%p (p<=size)
 * b.解决冲突采用开放定址法，即f2(key)=(f(key)+i)%p (1<=i<=size-1)
 */

// 引用：https://mp.weixin.qq.com/s/j2j9gS62L-mmOH4p89OTKQ
...
public class HashSearch {
     ...
     * 若在表中找到一个开放地址，则将待插入的节点插入到其中，则插入成功
     */
    public void insertHashTable(HashTable[] ha, int p, int size, int key) {
        int i = 1;
        int addr = 0;
        addr = key % p; // 通过哈希函数获取哈希地址
        if (ha[addr].key == NULLKEY || ha[addr].key == DELKEY) { //如果没有冲突，则直接插入
            ha[addr].key = key;
            ha[addr].count = 1;
